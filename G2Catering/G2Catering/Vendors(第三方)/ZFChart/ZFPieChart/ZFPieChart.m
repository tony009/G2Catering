...
/** 记录现在点击的模块 */
@property (nonatomic, assign) UIView *currrentBackGound;
...
//    _radius = _maxRadius * ZFPieChartCirqueRatio;
    ...
    _percentOnChartFontSize = 15.f;
    _extendLength = _radius * ZFPieChartCirqueRatio;
    _originHeight = self.frame.size.height;
    _pieCenter = CGPointMake(self.center.x - 50, CGRectGetHeight(self.topicLabel.frame) + NAVIGATIONBAR_HEIGHT + _radius - 40);
    _opacity = 1.f;
...
//        CGFloat yPos = _piePatternType == kPieChartPatternTypeForCirque ? _pieCenter.y + _radius * 1.75 + height * i : _pieCenter.y + _radius * 2.4 + height * i;
          CGFloat yPos =  height * i + 50;
        
        
        //装载容器
        UIView * background = [[UIView alloc] initWithFrame:CGRectMake(220, yPos, self.frame.size.width, height)];
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    ...
    for (NSInteger i = 0; i < _startAngleArray.count; i++) {
        ...
        for (UIView *background in self.subviews) {
            ...
            if (background.tag == i + PieChartDetailBackgroundTag) {
                
                self.currrentBackGound = background;
                
                [UIView animateWithDuration:1 animations:^{
                    
                    self.currrentBackGound.transform = CGAffineTransformMakeScale(1.1, 1.1);
                }];
            }
        }
...
- (void)showTranslucencePathAction:(UITapGestureRecognizer *)sender{
    ...
    for (UIView *background in self.subviews) {
        ...
        if (background.tag == sender.view.tag) {
